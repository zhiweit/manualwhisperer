...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--kb-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--kb-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[opened]:animate-in data-[closed]:animate-out data-[swipe=end]:animate-out data-[closed]:fade-out-80 data-[closed]:slide-out-to-right-full data-[opened]:slide-in-from-top-full data-[opened]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
    ...
    >
      <Switch>
        <Match when={props.state === "pending"}>
          <Show when={options.loading} fallback={<>Loading...</>}>
            {options.loading}
          </Show>
        </Match>
        <Match when={props.state === "fulfilled"}>
          {(_) => {
            const success = () => options.success?.(props.data!);

            return (
              <div class="flex flex-col gap-1">
                <Show when={success()?.title}>
                  {(title) => <ToastTitle>{title()}</ToastTitle>}
                </Show>
                <Show when={success()?.description}>
                  {(description) => (
        ...
        <Match when={props.state === "rejected"}>
          {(_) => {
            const error = () => options.error?.(props.error!);
            return (
              <div class="flex flex-col gap-1">
                <Show
                  when={error()}
                  fallback={<ToastTitle>Unknown error</ToastTitle>}
                >
                  {(error) => <ToastTitle>{error().title}</ToastTitle>}
                </Show>
